...
export async function getChargerStatusRedis(chargerId: number): Promise<ApiChargerStatus | null> {
  await ensureRedis();
  const value = await redis.get(rkeys.chargerStatus(chargerId));
  if (!value) return null;
  if (value === "available" || value === "in_use" || value === "outage") return value;
  return null;
}
